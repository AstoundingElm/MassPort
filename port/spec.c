...
fn_begin() {
        Function_Builder fn = {
                .stack_reserve = 0x0,
                .buffer = make_buffer(1024, MAP_PRIVATE | MAP_ANONYMOUS),
...
fn_arg(
       ...
       Descriptor descriptor
        ...
        // @Volatile @ArgumentCount
        assert(!"More than 4 argument_list are not supported at the moment.");
...
call_1(
       ...
       Value *arg0
        ...
        assert(to_call->descriptor.type == Descriptor_Type_Function);
        Descriptor_Function *descriptor = &to_call->descriptor.function;
        assert(descriptor->argument_list);
        assert(descriptor->argument_count == 1);
        // FIXME type check argument_list
        
        encode(&builder->buffer, (Instruction) {mov, {descriptor->argument_list[0].operand, arg0->operand}});
        encode(&builder->buffer, (Instruction) {mov, {rax, to_call->operand}});
        encode(&builder->buffer, (Instruction) {call, {rax, 0}});
...
make_is_non_zero() {
        Function_Builder fn = fn_begin();
        encode(&fn.buffer, (Instruction) {cmp, {rdi, imm32(0)}});
